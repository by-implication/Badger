...
case class PGLTree(list: Seq[String]) extends PGobject {
  setType("ltree")
...
object PGLTree {
    ...
    Column[PGLTree](transformer = { (value, meta) =>
      ...
      value match {
        case pgo: PGobject => Right {
          ...
          val list: Seq[String] = if(v.isEmpty) Nil else v.split('.')
          PGLTree(list)
        }
  ...
  implicit def seqToPGLTree(list: Seq[String]): PGLTree = new PGLTree(list)
